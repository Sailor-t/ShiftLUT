...
def ycbcr2rgb(img):
    '''
    Converts a YCbCr image (in Matlab format) back to RGB.
    Input:
        img: np.ndarray, shape (..., 3)
            - dtype can be float in [0,1] or uint8 in [0,255]
            - assumes input in YCbCr with Matlab-style offsets
    Output:
        RGB image, same dtype and shape
    '''
    ...
    # Subtract offsets
    img -= np.array([16, 128, 128], dtype=np.float32)

    # Inverse transform matrix (from BT.601)
    mat = np.array([
        [ 0.00456621,  0.00000000,  0.00625893],
        [ 0.00456621, -0.00153632, -0.00318811],
        [ 0.00456621,  0.00791071,  0.00000000]
    ]) * 255.0

    # Apply transform
    rlt = img @ mat.T  # shape (..., 3)

    # Clip and convert back to input type
