...
from data_ddp_deblocking import Provider, DBBenchmark
import model_2 as Model

sys.path.insert(0, "../../")  # run under the project directory
from common.option import TrainOptions
from common.utils import PSNR, logger_info, _rgb2ycbcr, modcrop, bPSNR
    ...
    with torch.no_grad():
        for dataset_name in datasets:
            ...
            for file_name in files_per_rank:
                ...
                val_H = valid.ims[key]
                val_L = valid.ims[key + 'x%d' % opt.qf]
                ...
                psnr_val = PSNR(output[:, :, 0], img_gt[:, :, 0], 0)
                psnrb_val = bPSNR(output, img_gt, 0)

                local_psnrs.append(psnr_val)
                local_psnrbs.append(psnrb_val)

            # 将本地 psnr 转为 tensor
            local_tensor = torch.tensor(local_psnrs, dtype=torch.float32, device=rank)
            local_tensorb = torch.tensor(local_psnrbs, dtype=torch.float32, device=rank)
            ...
            if len(local_psnrs) < max_len:
                ...
                local_tensorb = torch.cat([local_tensorb, pad], dim=0)
                

            # 收集所有进程的 PSNR
            all_psnrs = [torch.zeros(max_len, dtype=torch.float32, device=rank) for _ in range(world_size)]
            all_psnrsb = [torch.zeros(max_len, dtype=torch.float32, device=rank) for _ in range(world_size)]
            dist.all_gather(all_psnrs, local_tensor)
            dist.all_gather(all_psnrsb, local_tensorb)
            # print(all_psnrs)
            # 只在 rank 0 汇总和记录
            if rank == 0:
                ...
                valid_psnrsb = [all_psnrsb[i][:sizes[i]] for i in range(world_size)]
                all_psnrs_flatb = torch.cat(valid_psnrsb)
                avg_psnrb = all_psnrs_flatb.mean().item()        

                logger.info(f'Iter {iter} | Dataset {dataset_name} | AVG Val PSNR: {avg_psnr:.2f}/{avg_psnrb:.2f}')
                writer.scalar_summary(f'PSNR_valid/{dataset_name}', avg_psnr, iter)
                writer.scalar_summary(f'PSNRb_valid/{dataset_name}', avg_psnrb, iter)
...
if __name__ == "__main__":
    ...
    if opt.startIter > 0:
        ...
        load_model, load_iter = (
            'ShiftLUT_F_deblocking_10', 200000
            )
        lm = torch.load(
