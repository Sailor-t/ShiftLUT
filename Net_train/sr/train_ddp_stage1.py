    ...
    with torch.no_grad():
        for dataset_name in datasets:
            ...
            for file_name in files_per_rank:
                ...
                val_H = np.asarray(valid.ims[key])
                ...
                psnr_val = PSNR(_rgb2ycbcr(img_gt)[:, :, 0], _rgb2ycbcr(output)[:, :, 0], 4)
...
if __name__ == "__main__":
    ...
    model_G = model(scale=opt.scale, stacks=opt.stack, use_shift=opt.use_shift, msb_base=opt.msb_base, cnum=opt.cnum)
